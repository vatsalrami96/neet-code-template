...
In their Chrome LeetCode tab (see "LeetCode access" in AGENTS.md - Chrome, not the built-in pane) run (from `scripts/leetcode.js`)
...
Run `submissionCode(<id>)` (or `codesFor([<slug>])`). Write `solutions/<section>/<lcid padded to 4>-<slug>.py` with the header format from `AGENTS.md`
